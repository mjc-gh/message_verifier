...
impl Verifier {
    ...
    pub fn verify(&self, message: &str) -> Result<Vec<u8>> {
        let (encoded_data, signature) = split_by_dashes(&message)?;
...
impl Encryptor {
    ...
    pub fn decrypt_and_verify(&self, message: &str) -> Result<Vec<u8>> {
        ...
        let (encoded_cipher_text, encoded_iv) = split_by_dashes_from_u8_slice(&decoded)?;

        let cipher_text = encoded_cipher_text.from_base64()?;
        let iv = encoded_iv.from_base64()?;
